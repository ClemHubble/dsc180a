...
## ICE
This branch implements clarification generation methods from Input Clarification Ensembling (ICE) on Llama-3.1-8B-Instruct. Input Clarification Ensembling is a technique where multiple responses from clarifications are ensembled together, and then used to evaluate uncertainty through their distributions. 

Uncertainty evaluation in LLMs is important as it helps us understand how confident a model is in its predictions, which is crucial for reliability, safety, and decision-making. Without quantifying uncertainty, LLMs may output answers that seem correct, but in reality are based on hallucination. By accounting for these errors, we can detect model weaknesses, ambiguous inputs, and help strengthen models overall. 
