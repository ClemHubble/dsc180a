...
class TriviaQADataset(LMDataset):
    ...
    zero_shot_preamble = """Answer the question below. Provide a concise answer. Question: {question} Answer:"""
    sample_preamble = (
        """Answer the question below. Provide a concise answer. Question: {question} Answer: {answer}"""
    )
    qa_preamble = """Provide a concise answer. Question: {question} Answer:"""
    ...
    def s2s_collate_fn(self, batch):
        ...
        targets_aliases = []
        for e in batch:
            # Get aliases, fallback to main value if aliases are empty
            aliases = [normalize_answer(a) for a in e["answer"]["aliases"]]
            if not aliases:  # If aliases list is empty
                aliases = [normalize_answer(e["answer"]["value"])]
            targets_aliases.append(self._tokenize_prompts_batch(aliases))
...
class SQuADV2Dataset(LMDataset):
    """
    SQUAD v2.0 dataset for extractive question answering.
    ...
    def __init__(
        ...
        dset = load_dataset("squad_v2")
        self.n_shot = n_shot
        
        super().__init__(dset, tokenizer, add_space, max_seq_len=max_seq_len)
        
        # Check if tokenizer uses chat template
        self.use_chat_template = hasattr(tokenizer, 'chat_template') and tokenizer.chat_template is not None

    zero_shot_preamble = """Answer the question based on the context. Provide a concise answer. If the question cannot be answered from the context, respond with "unanswerable".

Context: {context}
Question: {question}
Answer:"""

    sample_preamble = """Answer the question based on the context. Provide a concise answer. If the question cannot be answered from the context, respond with "unanswerable".

Context: {context}
Question: {question}
Answer: {answer}"""

    qa_preamble = """Context: {context}
Question: {question}
Answer:"""
    ...
    def _select_few_shot_samples(self, dset, n):
        ...
        sampled_contexts = [dset["context"][i] for i in selected_indices]
        sampled_questions = [dset["question"][i] for i in selected_indices]
        sampled_answers = []
        for i in selected_indices:
            if len(dset["answers"][i]["text"]) > 0:
                sampled_answers.append(normalize_answer(dset["answers"][i]["text"][0]))
            else:
                sampled_answers.append("unanswerable")
    ...
    def _generate_n_shot_preamble(self):
        example_str = ""
        for i in range(self.n_shot):
            example_str += f"Example: Context: {self.few_shot_samples['context'][i]} Question: {self.few_shot_samples['question'][i]} Answer: {self.few_shot_samples['answer'][i]}. "
    ...
    def _format_prompts(self, batch):
        ...
        for e in batch:
            if self.use_chat_template:
                ...
                if self.n_shot > 0:
                    self._select_few_shot_samples(self.dset["train"][:100], self.n_shot)
                    user_message = self._generate_n_shot_preamble() + f"Context: {e['context']}\nQuestion: {e['question']}"
                else:
                    user_message = f"Answer the question based on the context. Provide a concise answer. If the question cannot be answered, respond with 'unanswerable'.\n\nContext: {e['context']}\nQuestion: {e['question']}"
                ...
                if self.n_shot > 0:
                    self._select_few_shot_samples(self.dset["train"][:100], self.n_shot)
                    prompt = self._generate_n_shot_preamble()
                    prompts.append(prompt + self.qa_preamble.format(context=e["context"], question=e["question"]))
                else:
                    prompt = self.zero_shot_preamble
                    prompts.append(prompt.format(context=e["context"], question=e["question"]))
    ...
    def _format_samples(self, batch):
        prompts = []
        for e in batch:
            if len(e["answers"]["text"]) > 0:
                answer = normalize_answer(e["answers"]["text"][0])
            else:
                answer = "unanswerable"
    ...
    def s2s_collate_fn(self, batch):
        ...
        # Handle multiple possible answers and unanswerable questions
        targets = []
        targets_aliases = []
        for e in batch:
            if len(e["answers"]["text"]) > 0:
                # Answerable question
                primary_answer = normalize_answer(e["answers"]["text"][0])
                targets.append(primary_answer)
                # Add all answer variants
                aliases = [normalize_answer(ans) for ans in e["answers"]["text"]]
                targets_aliases.append(self._tokenize_prompts_batch(aliases))
            else:
                # Unanswerable question
                targets.append("unanswerable")
                targets_aliases.append(self._tokenize_prompts_batch(["unanswerable"]))
        
        targets = self._tokenize_prompts_batch(targets)
        return prompts, targets, targets_aliases
    ...
    def s2s_train_collate_fn_ori(self, batch):
        ...
        # Create labels with masked prompt tokens
        tokenized_targets = tokenized_prompts.copy()
        
        if self.use_chat_template:
            # Mask everything before assistant's response
            assistant_header_ids = self.tokenizer.encode(
                "<|start_header_id|>assistant<|end_header_id|>\n\n",
                add_special_tokens=False
            )
            
            for i, input_ids in enumerate(tokenized_targets['input_ids']):
                assistant_start = -1
                for j in range(len(input_ids) - len(assistant_header_ids)):
                    if input_ids[j:j+len(assistant_header_ids)].tolist() == assistant_header_ids:
                        assistant_start = j + len(assistant_header_ids)
                        break
                
                if assistant_start > 0:
                    tokenized_targets['input_ids'][i][:assistant_start] = -100
        
        return tokenized_prompts, tokenized_targets


SQuADV2 = SQuADV2Dataset


class TruthfulQADataset(LMDataset):
    """
    TruthfulQA dataset for testing truthfulness of model answers.
    Tests whether models generate truthful answers to questions where humans might be misled.
    
    Note: TruthfulQA only has 'validation' split, so we split it into train/test.
    """
    def __init__(
        self,
        tokenizer: AutoTokenizer,
        ...
        dset = load_dataset("truthful_qa", "generation")
        self.n_shot = n_shot
        self.multianswer = multianswer
        
        # TruthfulQA only has 'validation' split (817 examples)
        # Split it into train (80%) and test (20%)
        if 'train' not in dset:
            val_data = dset['validation']
            # Split: first 650 for train, last 167 for test
            train_size = int(len(val_data) * 0.8)
            dset['train'] = val_data.select(range(train_size))
            dset['test'] = val_data.select(range(train_size, len(val_data)))
            dset['validation'] = dset['test']  # Use test as validation
        
        super().__init__(dset, tokenizer, add_space, max_seq_len=max_seq_len)
        
        # Check if tokenizer uses chat template (for Llama-3.x-Instruct models)
        self.use_chat_template = hasattr(tokenizer, 'chat_template') and tokenizer.chat_template is not None

    zero_shot_preamble = """Answer the following question truthfully and accurately. Provide a concise answer.

Question: {question}
Answer:"""

    sample_preamble = """Answer the following question truthfully and accurately. Provide a concise answer.

Question: {question}
Answer: {answer}"""

    qa_preamble = """Question: {question}
Answer:"""
    ...
    def _select_few_shot_samples(self, dset, n):
        ...
        sampled_answers = []
        for i in selected_indices:
            # Use the best answer (first correct answer)
            if len(dset["correct_answers"][i]) > 0:
                sampled_answers.append(normalize_answer(dset["correct_answers"][i][0]))
            else:
    ...
    def _generate_n_shot_preamble(self):
        example_str = ""
        for i in range(self.n_shot):
            example_str += f"Example: Question: {self.few_shot_samples['question'][i]} Answer: {self.few_shot_samples['answer'][i]}. "
    ...
    def _format_prompts(self, batch):
        ...
        for e in batch:
            if self.use_chat_template:
                # Use Llama chat format for instruction-tuned models
                if self.n_shot > 0:
                    self._select_few_shot_samples(self.dset["validation"][:100], self.n_shot)
                    user_message = self._generate_n_shot_preamble() + f"Question: {e['question']}"
                else:
                    user_message = f"Answer the following question truthfully and accurately. Provide a concise answer.\n\nQuestion: {e['question']}"
                
                messages = [{"role": "user", "content": user_message}]
                # Apply chat template, but don't add generation prompt yet (done in tokenization)
                prompt = self.tokenizer.apply_chat_template(
                    messages,
                    tokenize=False,
                ...
                # Fallback to simple format for non-chat models
                if self.n_shot > 0:
                    self._select_few_shot_samples(self.dset["validation"][:100], self.n_shot)
                    prompt = self._generate_n_shot_preamble()
                    prompts.append(prompt + self.qa_preamble.format(question=e["question"]))
    ...
    def _format_samples(self, batch):
        prompts = []
        for e in batch:
            # Use the first correct answer for training
            if len(e["correct_answers"]) > 0:
                answer = normalize_answer(e["correct_answers"][0])
            else:
                answer = ""
    ...
    def s2s_collate_fn(self, batch):
        ...
        # Handle multiple correct and incorrect answers
        targets = []
        targets_aliases = []
        for e in batch:
            if len(e["correct_answers"]) > 0:
                # Primary target is the first correct answer
                targets.append(normalize_answer(e["correct_answers"][0]))
                # All correct answers as aliases
                if self.multianswer:
                    aliases = [normalize_answer(ans) for ans in e["correct_answers"]]
                else:
                    aliases = [normalize_answer(e["correct_answers"][0])]
                targets_aliases.append(self._tokenize_prompts_batch(aliases))
            else:
                targets.append("")
                targets_aliases.append(self._tokenize_prompts_batch([""]))
    ...
    def s2s_train_collate_fn_ori(self, batch):
        ...
        # Create labels with masked prompt tokens (only compute loss on answer)
        tokenized_targets = tokenized_prompts.copy()
        
        if self.use_chat_template:
            # For chat models, mask everything before the assistant's response
            # The assistant response starts after "<|start_header_id|>assistant<|end_header_id|>\n\n"
            assistant_header_ids = self.tokenizer.encode(
                "<|start_header_id|>assistant<|end_header_id|>\n\n",
                add_special_tokens=False
            )
