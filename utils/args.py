...
def add_experiment_args(parser: ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        type=bool,
        default=True,
        help="Whether to handle multiple answer variants for open-ended QA datasets. Default is True.",
    )
    parser.add_argument(
        "--subset-size",
        type=int,
        default=0,
        help="Size of the training subset to use. Set to 0 to use the full dataset. Default is 0.",
    )
    ...
    # Generative evaluation arguments (for open-ended QA)
    parser.add_argument(
        "--eval-mode",
        type=str,
        default="classification",
        choices=["classification", "generative"],
        help="Evaluation mode: 'classification' for multiple-choice QA, 'generative' for open-ended QA. Default is 'classification'.",
    )
    parser.add_argument(
        "--generative-eval-method",
        type=str,
        default="string_match",
        choices=["llm_judge", "nli", "string_match"],
        help="Method for evaluating generated answers: 'llm_judge' (LLM-as-judge), 'nli' (mutual entailment with NLI model), or 'string_match' (simple string matching). Default is 'string_match'.",
    )
    parser.add_argument(
        "--use-llm-judge",
        action="store_true",
        help="[DEPRECATED] Use LLM-as-judge for evaluating generated answers. Use --generative-eval-method llm_judge instead.",
    )
    parser.add_argument(
        "--llm-judge-model",
        type=str,
        default="meta-llama/Llama-3.1-8B-Instruct",
        help="Model to use as LLM judge for semantic equivalence evaluation. Default is Llama-3.1-8B-Instruct.",
    )
    parser.add_argument(
        "--nli-model",
        type=str,
        default="MoritzLaurer/DeBERTa-v3-large-mnli-fever-anli-ling-wanli",
        help="Model to use for NLI-based evaluation (mutual entailment). Options: 'microsoft/deberta-v3-large-mnli', 'roberta-large-mnli', 'facebook/bart-large-mnli'. Default is DeBERTa-v3-large.",
    )
    parser.add_argument(
        "--nli-threshold",
        type=float,
        default=0.5,
        help="Probability threshold for NLI entailment classification. Higher values are more strict. Default is 0.5.",
    )
    parser.add_argument(
        "--judge-gpu-id",
        type=int,
        default=None,
        help="Specific GPU ID to load the LLM judge on (e.g., 1 for cuda:1). If not specified, uses device_map='auto'. Useful for multi-GPU setups to separate training model and judge model.",
    )
    parser.add_argument(
        "--max-new-tokens",
        type=int,
        default=50,
        help="Maximum number of tokens to generate for open-ended QA. Default is 50.",
    )
    parser.add_argument(
        "--generation-temperature",
        type=float,
        default=0.7,
        help="Sampling temperature for text generation. Use 0.0 for greedy decoding. Default is 0.7.",
    )
    parser.add_argument(
        "--generation-top-p",
        type=float,
        default=0.9,
        help="Nucleus sampling parameter (top-p) for text generation. Default is 0.9.",
    )
    parser.add_argument(
        "--generation-do-sample",
        action="store_true",
        default=True,
        help="Enable sampling for text generation (vs greedy decoding). Default is True.",
    )
