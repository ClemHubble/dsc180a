...
def main(args=None):
    ...
    # Pass tokenizer to model if available (for embedding resizing)
    tokenizer = getattr(dataset, 'tokenizer', None)
    if tokenizer is not None:
        print(f"✅ Tokenizer found: vocab_size={tokenizer.vocab_size}, len={len(tokenizer)}")
    else:
        print("⚠️ WARNING: No tokenizer found in dataset!")
    model = get_model(args, accelerator, tokenizer=tokenizer)
